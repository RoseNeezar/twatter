...
interface IProfileFollowing {
  backUrl: string;
}
const ProfileFollowing: FC<IProfileFollowing> = ({ backUrl }) => {
  const currentUserProfile = useAppSelector(
    (state: RootState) => state.user.currentUserProfile
  );

  const currentUserFollowingProfile = useAppSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getUserProfileFollowing(String(currentUserProfile?.id)));
  }, [dispatch, getUserProfileFollowing, currentUserProfile, currentUser]);

  return (
    <>
      {currentUserFollowingProfile?.following?.map((re) => (
        <div
          key={re.id}
